...
public:
    ...
    void add_edge(const char c, const Edge &e) { edges[c] = e; }
    Edge const* get_edge(const char c) const {
        return edges.find(c) != edges.end() ? &edges.at(c) : nullptr;
    }
    Edge* get_edge(const char c) {
        return edges.find(c) != edges.end() ? &edges.at(c) : nullptr;
    }
    std::map<char, Edge> get_edges() const { return edges; }
