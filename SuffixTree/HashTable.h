...
private:
    ...
    insert(const key_type &key, const mapped_type &mapped_value, const size_type &hash_idx) {
        {
...
public:
    ...
    HashTable(size_type size, std::function<std::size_t(const key_type &, size_type)> hash) : size_{size},
                                                                                              left_{size_} {
        table_.resize(size_);
        hash_ = hash;
    }
